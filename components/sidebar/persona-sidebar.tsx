...
// Agent IDs
const AGENTS = {
  amogh: "agent_5201kemqecb1efr8c3wg3efbqgk3",
...
const PERSONAS: Persona[] = [
  {
    ...
    description: "Hear directly from a victim of a collision",
  ...
  },
  {
    ...
    description: "Talk to the Mayor about the city's safety",
  ...
  },
  {
    ...
    description: "Learn from a technical expert in the field",
    ...
    try {
      ...
      if (selectedHotspot) {
        ...
        if (clusterData?.safetyAudit) {
          ...
          dynamicVariables.walkability_score =
            audit.walkabilityScore.toString();
          ...
          dynamicVariables.crosswalk_visibility_score =
            audit.metrics.crosswalkVisibility.toString();
          dynamicVariables.bike_infrastructure_score =
            audit.metrics.bikeInfrastructure.toString();
          dynamicVariables.pedestrian_infrastructure_score =
            audit.metrics.pedestrianInfrastructure.toString();
          dynamicVariables.traffic_calming_score =
            audit.metrics.trafficCalming.toString();

          // Safety flaws summary
          if (audit.flaws.length > 0) {
            const highSeverityFlaws = audit.flaws.filter(
              (f) => f.severity === "high",
            );
            const mediumSeverityFlaws = audit.flaws.filter(
              (f) => f.severity === "medium",
            );
            const lowSeverityFlaws = audit.flaws.filter(
              (f) => f.severity === "low",
            );

            dynamicVariables.total_flaws = audit.flaws.length.toString();
            dynamicVariables.high_severity_flaws =
              highSeverityFlaws.length.toString();
            dynamicVariables.medium_severity_flaws =
              mediumSeverityFlaws.length.toString();
            dynamicVariables.low_severity_flaws =
              lowSeverityFlaws.length.toString();

            // List of flaw titles
            dynamicVariables.flaw_titles = audit.flaws
              .map((f) => f.title)
              .join("; ");
          }
          ...
          if (audit.suggestions.length > 0) {
            const highPrioritySuggestions = audit.suggestions.filter(
              (s) => s.priority === "high",
            );
            dynamicVariables.total_suggestions =
              audit.suggestions.length.toString();
            dynamicVariables.high_priority_suggestions =
              highPrioritySuggestions.length.toString();
            dynamicVariables.suggestion_titles = audit.suggestions
              .map((s) => s.title)
              .join("; ");
          }
          ...
          if (audit.infrastructureGaps.length > 0) {
            dynamicVariables.missing_infrastructure =
              audit.infrastructureGaps.join(", ");
            dynamicVariables.infrastructure_gap_count =
              audit.infrastructureGaps.length.toString();
          }
        }
        ...
        if (
          clusterData &&
          clusterData.images.history &&
          clusterData.images.history.length > 0
        ) {
