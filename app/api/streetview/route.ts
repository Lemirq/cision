...
export async function GET(request: NextRequest) {
  ...
  const date = searchParams.get("date"); // Date parameter for historical imagery (YYYY-MM or YYYY-MM-DD)
  ...
  // Build the Street View URL with optional date parameter for historical imagery
  // Date format: YYYY-MM or YYYY-MM-DD
  let streetViewUrl = `https://maps.googleapis.com/maps/api/streetview?size=${size}&location=${lat},${lng}&heading=${heading}&fov=${fov}&pitch=${pitch}&scale=${scale}`;

  // Add date parameter if provided (for historical imagery from crash time)
  if (date) {
    console.log("Date parameter:", date);
    streetViewUrl += `&date=${date}`;
  }
  ...
  try {
    ...
    if (
      ...
      console.error(
        "Street View API returned error:",
    ...
    console.error(
      "Unexpected Street View API response:",
